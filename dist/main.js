import { IncomingIBLTReconciliation, OutgoingIBLTReconciliation, } from "./set_reconciliation/reconcile.js";
...
////////
console.log("-------");
let AW0 = new AddWinsSet((one, two) => one == two, 0);
let AW1 = new AddWinsSet((one, two) => one == two, 1);
...
let setAa = AW0.decompose();
let setBb = AW1.decompose();
let outgoing2 = new OutgoingIBLTReconciliation(setAa, SerializablePair2);
let incoming2 = new IncomingIBLTReconciliation(setBb, outgoing2.lookupTable);
incoming2.reconcileLocalSet();
//console.log(setBb);
const response2 = ValueSet.fromSet(incoming2.computeTransmission(), (pairL, pairR) => pairL.left == pairR.left && pairL.right == pairR.right);
