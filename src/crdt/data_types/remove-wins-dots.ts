import { Causal } from "../causal-state.js";
import { DotMap } from "../dots/dot-map.js";
import { DotSet } from "../dots/dot-set.js";
import { Dot } from "../dots/dot.js";

export class RemoveWinsSet<T> extends Causal<DotMap<T, DotMap<boolean, DotSet>>> {
  keyEquals: (keyL: T, keyR: T) => boolean;
  vCtor: new (...args: any[]) => DotMap<boolean, DotSet>;
  lastDot: Dot;

  constructor(
    keyEquals: (keyL: T, keyR: T) => boolean,
    vCtor: new (...args: any[]) => DotMap<boolean, DotSet>,
    replicaId: number
  ) {
    super(new DotMap(keyEquals, vCtor));
    this.vCtor = vCtor;
    this.keyEquals = keyEquals;
    this.lastDot = new Dot(replicaId, 0);
  }

  add(element: T): void {
    let currentValue: DotMap<boolean, DotSet>;
    let d: DotSet = new DotSet();
    let dot: Dot = this.next();
    let store: DotMap<T, DotMap<boolean, DotSet>> = new DotMap(this.keyEquals, this.vCtor);
    let valueMap: DotMap<boolean, DotSet> = new DotMap(this.booleanEquals, DotSet);
    d.add(dot);

    if (this.store.contains(element)) {
      currentValue = this.store.get(element)!.clone();
    } else {
      currentValue = new DotMap(this.booleanEquals, DotSet);
    }

    valueMap.set(true, d);
    store.set(element, valueMap);

    this.store = store;
    this.causalContext = d.union(DotSet.fromSet(currentValue.dots()));
  }

  remove(element: T): void {
    let currentValue: DotMap<boolean, DotSet>;
    let d: DotSet = new DotSet();
    let dot: Dot = this.next();
    let store: DotMap<T, DotMap<boolean, DotSet>> = new DotMap(this.keyEquals, this.vCtor);
    let valueMap: DotMap<boolean, DotSet> = new DotMap(this.booleanEquals, DotSet);
    d.add(dot);

    if (this.store.contains(element)) {
      currentValue = this.store.get(element)!.clone();
    } else {
      currentValue = new DotMap(this.booleanEquals, DotSet);
    }

    valueMap.set(false, d);
    store.set(element, valueMap);

    this.store = store;
    this.causalContext = d.union(DotSet.fromSet(currentValue.dots()));
  }

  clear(): void {
    const dots = this.store.dots();
    this.store = new DotMap(this.keyEquals, this.vCtor);
    this.causalContext = DotSet.fromSet(dots);
  }

  elements(): Set<T> {
    let output: Set<T> = new Set();

    for (const [key, val] of this.store.internal) {
      let hasFalse = false;
      for (const [keyInner, valInner] of val.internal) {
        if (!keyInner) {
          hasFalse = true;
        }
      }

      if (!hasFalse) {
        output.add(key);
      }
    }

    return output;
  }

  booleanEquals(boolL: boolean, boolR: boolean) {
    return boolL === boolR;
  }

  next(): Dot {
    const dot = new Dot(this.lastDot.replica, this.lastDot.sequence_number + 1);
    this.lastDot = dot;
    return dot;
  }
}
